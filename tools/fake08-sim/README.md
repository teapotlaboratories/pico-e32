...
`components/z8lua`), driven headlessly for **solving and verifying** Celeste input plans off-device, and for
rendering a **pixel-perfect video** to compare against the bench camera. It runs the real cart in the real
VM, so there is no hand-written physics model to keep in sync — this is the ground truth the Python twin
(`tools/celeste_solver/celeste_sim.py`) was validated against.
...
| `render_run.py` | Render the two solved rooms to `sim_run.mp4`. |

## Build + use

```sh
make -C tools/fake08-sim                      # -> libfake08sim.so
python3 -c "import sys; sys.path.insert(0,'tools/fake08-sim'); import fake08sim as S; \
  S.init('assets/celeste.p8'); print(S.spawn(0,0))"     # -> {x:8, y:96, rx:0, ry:0, dj:1}  (matches the device)
python3 tools/fake08-sim/render_run.py sim_run.mp4       # pixel-perfect run video
```
...
3. **Deliver** the plan to the device (`tools/celeste_playtest.py`, frame-synced serial) and verify via
   telemetry + camera.
...
(`tools/celeste_solver`), which **this sim has proven exact**; the sim is the validator + renderer. Making
savestates work (for a fully VM-native search) is the open follow-up.
