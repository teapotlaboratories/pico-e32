...
little as possible, and **never delete a file merely to exclude it from the build**.
Prefer, in order: (1) **exclude at the build layer** — e.g. keep an uncompiled unit at its
upstream `.c` extension so a `*.cpp` glob skips it, or leave it out of the source list;
(2) **guard** target-specific behaviour behind a compile-time `#if` rather than ripping code
out; (3) keep upstream's own files (`README`, `makefile`, license, tests) in place unless
they actively break the build. Deletion is a last resort; every removal — and every edit —
is recorded in the component's vendoring notes (e.g. `LOCAL_PATCHES.md`). This keeps upstream
rebases clean and the divergence auditable.
