...
| `setTargetFps` / `waitForTargetFps` | `host.h:107,117` | `BittBoyHost.cpp:252,364` | `esp_timer` + `vTaskDelay` instead of `SDL_GetTicks`/`SDL_Delay` — divergence D2. Local addition: an opt-in `-D SHOW_FPS` on-screen HUD (`board_lcd_draw_fps`) measures the loop rate here; it **repaints every tick** (not just on value change) so it survives a game that blits over its `(8,8)` spot — required on the P4, where the game fills the panel width. Off by default. |
...
| `listcarts`/`listdirs`/`getCartDirectory` (impl); `logFilePrefix`/`customBiosLua` (stub) | `host.h:130,146,144;133,135` | `platform/gcw0/source/ODHost.cpp:642` (listcarts), `…:680` (listdirs) | `listcarts` scans `_cartDirectory` (the SD mount) for `.p8`/`.p8.png`, lower-casing names (defensive: matches any case, incl. an 8.3 fallback); `getCartDirectory` returns `_cartDirectory`. **`listdirs` now implemented** (ported from gcw0 `ODHost.cpp` `listdirs`): scans `_cartDirectory`, `opendir`-probing each entry to detect subdirectories (FatFs on esp-idf doesn't populate `dirent.d_type`) — this lights up the DefaultCart/launcher folder navigation (the `__cd`/`__listdirs`/`ChangeDirectory` chain was already present). `logFilePrefix`/`customBiosLua` still return `""`. |
