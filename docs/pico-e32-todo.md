...
- **Guition JC4880P443C (ESP32-P4) board:** [`hardware/pico-e32-guition-jc4880p443c-p4.md`](hardware/pico-e32-guition-jc4880p443c-p4.md) — a SECOND, **RISC-V** target alongside the S3 boards; portable across S3+P4 (`GP-1`…`GP-7`). **`GP-1`–`GP-4` DONE + `GP-7` core DONE (2026-07-29): real Celeste runs on the P4** — the fake-08 runtime on RISC-V, 480×800 ST7701S panel over MIPI-DSI, driven by serial input, all camera-verified. Board boots, 32 MB PSRAM + early v1.3 silicon confirmed; needed a [RISC-V z8lua fix](runtime/pico-e32-fake08-port.md). `GP-5` input partial (serial ✅, GT911 touch controller-only — HITL pending); `GP-6` audio backlogged; GP-7 fps (Gate-2 re-validation) pending. Apps: `firmware/pico-e32-p4-hello` (bring-up) + `pico-e32-fake08` (Celeste).
