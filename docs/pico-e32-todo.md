...
Detail lives in **[`firmware/pico-e32-wifi-networking.md`](firmware/pico-e32-wifi-networking.md)**. Status at a glance:

| item | what | status |
|---|---|---|
| `WC-1` | S3 WiFi foundation — `components/wifi`, scan/connect/persist, on-screen keyboard | ✅ done |
| `WC-2` | case-visible text — solved with colour, not glyphs (capitals take a fixed cyan) | ✅ done |
