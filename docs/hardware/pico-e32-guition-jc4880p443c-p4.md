...
| **TF/microSD (SDMMC)** | slot-0 IO_MUX, 4-bit: **CLK GPIO43 / CMD GPIO44 / D0 GPIO39 / D1 GPIO40 / D2 GPIO41 / D3 GPIO42**; card 3.3 V rail from **on-chip LDO channel VO4** (`sd_pwr_ctrl` `ldo_chan_id=4`) — must be brought up as `host.pwr_ctrl_handle` **before** mount, else every init command times out (`send_op_cond 0x107`) | giltal/RetroESP32-P4 `odroid_sdcard.c` + IDF P4 defaults (`board_sd_mount` in `board.cpp`) |
