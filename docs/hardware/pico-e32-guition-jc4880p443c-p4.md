...
| **TF/microSD — how we drive it** | **SPI, not SDMMC** (`BOARD_HAS_SD` + `sdcard_spi`, same seam as the S3): **SCLK GPIO43 (CLK) / MOSI GPIO44 (CMD) / MISO GPIO39 (D0) / CS GPIO42 (D3)** on `SPI2_HOST`. Why: the P4 has **one** SDMMC host and the on-board **ESP32-C6 radio** needs it (slot 1, esp-hosted/SDIO) — SD (slot 0) and C6 cannot both init it, whichever comes up first locks the other out. Running the SD over SPI frees the SDMMC host entirely for the C6, so **WiFi + SD coexist**. (`BOARD_HAS_SDMMC`/`board_sd_mount` were removed; `board_sd_config` in `board.cpp` now powers VO4 + fills the SPI wiring.) | standard TF-in-SPI mapping; LDO rail per the row above (`board_sd_config` in `board.cpp`) |
